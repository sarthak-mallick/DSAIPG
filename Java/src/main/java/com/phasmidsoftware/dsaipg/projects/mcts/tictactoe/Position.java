 ...
 */
 ...
 public class Position {
     ...
     boolean threeInARow() {
         for (int i = 0; i < gridSize; i++){
             if (threeEqual(projectRow(i)) || threeEqual(projectCol(i)))
                 return true;
         }
         if (threeEqual(projectDiag(true)) || threeEqual(projectDiag(false)))
             return true;

         return false;
     }
 
     boolean threeEqual(int[] checkThree) {
         return checkThree[0] == checkThree[1] && checkThree[0] == checkThree[2] && checkThree[0] == last;
     }
